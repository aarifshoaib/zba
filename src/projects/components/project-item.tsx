...
const IbtikarItem = ({ data, navigation, index }) => {
  ...
  const navigateToDetails = () => {
    authContext.selectedProject(data);
      ...
      <TouchableOpacity onPress={navigateToDetails} style={{ flex: 1 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
          <View style={{ flexDirection: 'row' }}>
            <Text style={styles.ref}>{data.prj_code}-{data.prj_name}</Text>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'flex-end' }}>
            <Text style={styles.date}>{moment(data.created_on).format('DD-MMM-YYYY hh:mm')}</Text>
          </View>
        </View>
